package my_app.soar.MyPage.TagSelection
...
import my_app.soar.databinding.StepCsJobBinding
...
class Step2Fragment : Fragment(R.layout.step_cs_job) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // ✨ [수정] activityViewModel의 job 데이터가 변경될 때마다 UI를 초기화하도록 변경
        activityViewModel.job.observe(viewLifecycleOwner) { jobTag ->
            // 수정 모드일 때만 이 로직을 실행하여, 사용자가 새로 선택하는 값에 덮어쓰지 않도록 함
            if (activityViewModel.isEditMode.value == true) {
                viewModel.initializeSelection(jobTag?.tagId)
            }
        }
